...
class DatabaseLogin (context: Context) : SQLiteOpenHelper(context, "login.db", null, 1) {
    private val SQL_CREATE_ENTRIES_LOGIN =
        "CREATE TABLE ${Contract.LoginEntry.TABLE_NAME} (" +
                "${Contract.LoginEntry.COLUMN_NAME_ID} INTEGER PRIMARY KEY NOT NUL autoincrement," +
    ...
    fun insertLog(text: String) {
        ...
        values.put(Contract.LoginEntry.COLUMN_NAME_LOGIN, text)
        values.put(Contract.LoginEntry.COLUMN_NAME_PASSWORD, text)
        writableDatabase.insert(SQL_CREATE_ENTRIES_LOGIN, null, values)
    }

    fun getLogs(): Cursor {
        return readableDatabase
            .query(SQL_CREATE_ENTRIES_LOGIN, arrayOf(BaseColumns._ID, Contract.LoginEntry.COLUMN_NAME_LOGIN,
